...
# ldm_stable = StableDiffusionPipeline.from_pretrained("/content/drive/MyDrive/fine10000",scheduler=scheduler).to(device)
ldm_stable = StableDiffusionPipeline.from_pretrained("/content/drive/MyDrive/stable-diffusion-v1-4",scheduler=scheduler).to(device)
...
def run_and_display(prompts, controller, latent=None, run_baseline=False, generator=None, uncond_embeddings=None, verbose=True, description='',folder=None):
    ...
    if verbose:
        ...
        # ptp_utils.view_images(images[1],description=description,folder=folder)
    return images, x_t
...
parser.add_argument("--self_ratio", required=False, type=float)
parser.add_argument("--blend_word", required=False)
...
for style in styles:
    ...
    if len(words) > 1: 
        ...
        eq_params = {"words": tuple(words + ['style']), "values": tuple([2] * (len(words) + 1))}  
    else:  # if only one word
        prompts = [prompt, prompt + ", in " + style + " style, extra high quality and pleasing"]
        eq_params = {"words": (style, 'style', ), "values": (2,2,)} 
